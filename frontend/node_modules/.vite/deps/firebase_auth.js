import { A as deepEqual, B as isIE, C as LogLevel, D as FirebaseError, E as ErrorFactory, F as getUA, G as querystringDecode, H as isReactNative, I as isBrowserExtension, K as updateEmulatorBanner, L as isCloudWorkstation, M as getDefaultEmulatorHost, N as getExperimentalSetting, O as base64Decode, P as getModularInstance, R as isCloudflareWorker, T as Component, U as pingServer, V as isMobileCordova, W as querystring, c as _getProvider, f as _registerComponent, g as getApp, j as extractQuerystring, k as createSubscribe, n as SDK_VERSION, u as _isFirebaseServerApp, w as Logger, x as registerVersion, z as isEmpty } from "./index.esm-BgWuljMR.js";

//#region node_modules/@firebase/auth/dist/esm/index-1970027f.js
/**
* @license
...
var UserImpl = class UserImpl {
	...
	async _updateTokensIfNecessary(response, reload$1 = false) {
		let tokensRefreshed = false;
		if (response.idToken && response.idToken !== this.stsTokenManager.accessToken) {
			this.stsTokenManager.updateFromServerResponse(response);
			tokensRefreshed = true;
		}
		if (reload$1) await _reloadWithoutSaving(this);
		await this.auth._persistUserIfCurrent(this);
...
var PersistenceUserManager = class PersistenceUserManager {
	constructor(persistence, auth, userKey) {
		...
		const { config, name: name$1 } = this.auth;
		this.fullUserKey = _persistenceKeyName(this.userKey, config.apiKey, name$1);
		this.fullPersistenceKey = _persistenceKeyName("persistence", config.apiKey, name$1);
...
async function sendSignInLinkToEmail(auth, email, actionCodeSettings) {
	...
	function setActionCodeSettings(request$1, actionCodeSettings$1) {
		_assert(actionCodeSettings$1.handleCodeInApp, authInternal, "argument-error");
		if (actionCodeSettings$1) _setActionCodeSettingsOnRequest(authInternal, request$1, actionCodeSettings$1);
	}
...
var BrowserLocalPersistence = class extends BrowserPersistenceClass {
	...
	onStorageEvent(event, poll = false) {
		if (!event.key) {
			this.forAllChangedKeys((key$1, _oldValue, newValue) => {
				this.notifyListeners(key$1, newValue);
			});
			return;
		}
		const key = event.key;
		...
		const triggerListeners = () => {
			const storedValue$1 = this.storage.getItem(key);
			if (!poll && this.localCache[key] === storedValue$1) return;
			this.notifyListeners(key, storedValue$1);
		};
...
function getDocumentCookie(name$1) {
	const escapedName = name$1.replace(/[\\^$.*+?()[\]{}|]/g, "\\$&");
...
var CookiePersistence = class {
	...
	async _get(key) {
		...
		const name$1 = getCookieName(key);
		if (window.cookieStore) return (await window.cookieStore.get(name$1))?.value;
		return getDocumentCookie(name$1);
	}
	async _remove(key) {
		if (!this._isAvailable()) return;
		if (!await this._get(key)) return;
		const name$1 = getCookieName(key);
		document.cookie = `${name$1}=;Max-Age=34560000;Partitioned;Secure;SameSite=Strict;Path=/;Priority=High`;
	...
	_addListener(key, listener) {
		...
		if (window.cookieStore) {
			const cb = ((event) => {
				const changedCookie = event.changed.find((change) => change.name === name$1);
				if (changedCookie) listener(changedCookie.value);
				if (event.deleted.find((change) => change.name === name$1)) listener(null);
			});
			const unsubscribe$1 = () => window.cookieStore.removeEventListener("change", cb);
			this.listenerUnsubscribes.set(listener, unsubscribe$1);
			return window.cookieStore.addEventListener("change", cb);
		}
		let lastValue = getDocumentCookie(name$1);
		const interval = setInterval(() => {
			const currentValue = getDocumentCookie(name$1);
...
async function _openIframe(auth) {
	...
	const gapi$1 = _window().gapi;
	_assert(gapi$1, auth, "internal-error");
	return context.open({
		where: document.body,
		url: getIframeUrl(auth),
		messageHandlersFilter: gapi$1.iframes.CROSS_ORIGIN_IFRAMES_FILTER,
...
function _open(auth, url, name$1, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT) {
	...
	if (name$1) target = _isChromeIOS(ua) ? TARGET_BLANK : name$1;
	if (_isFirefox(ua)) {
...
var BrowserPopupRedirectResolver = class {
	...
	_initialize(auth) {
		...
		if (this.eventManagers[key]) {
			const { manager, promise: promise$1 } = this.eventManagers[key];
			if (manager) return Promise.resolve(manager);
			else {
				debugAssert(promise$1, "If manager is not set, promise should be");
				return promise$1;
			}
		}
