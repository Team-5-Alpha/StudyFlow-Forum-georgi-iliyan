...
const visibleTrendingPosts = computed(() => {
  if (isTrendingExpanded.value) {
    ...
    return topPosts.value.slice(0, 3);
  }
});
...
.logo { font-size: 28px; font-weight: 800; margin: 0 0 40px 15px; color: var(--color-white); letter-spacing: -0.5px; }
.dot { color: var(--color-accent); font-size: 32px; }
...
.user-mini-profile-link { text-decoration: none; margin-top: 20px; display: block; }
.user-mini-profile { display: flex; align-items: center; gap: 12px; padding: 12px 20px; background: rgba(255,255,255,0.05); border-radius: 50px; transition: background 0.2s; cursor: pointer; }
.user-mini-profile:hover { background: rgba(255,255,255,0.15); }
.avatar { width: 40px; height: 40px; background-color: var(--color-accent); color: var(--color-white); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; flex-shrink: 0; overflow: hidden; }
...
.trending-header { margin-top: 0; font-size: 18px; border-bottom: 2px solid var(--color-bg); padding-bottom: 10px; margin-bottom: 15px; color: var(--color-dark); display: flex; align-items: center; gap: 8px; }
.trending-icon { width: 24px; height: 24px; color: var(--color-accent); }
.trend-list { list-style: none; padding: 0; margin: 0; }
.trend-item { padding: 12px 0; border-bottom: 1px solid #f1f1f1; transition: background 0.2s; }
...
.show-more-btn:hover { color: var(--color-dark); text-decoration: underline; }
</style>